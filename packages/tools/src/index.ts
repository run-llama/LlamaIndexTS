    ...
    const metadata = transformObject(
      getObjectByAddress(tool.metadata, ToolMetadata),
      {
        ...
        argsKwargs: (argsKwargs) => {
          if (!argsKwargs) return null;
          const argsKwargsArray = __getArray(argsKwargs);
          return argsKwargsArray.map((argsKwargsRecord: any) => {
            return transformObject(
              getObjectByAddress(argsKwargsRecord, ArgsKwargsRecord),
              {
                key: __getString,
                value: (value) => {
                  const stringValue = __getString(value);
                  try {
