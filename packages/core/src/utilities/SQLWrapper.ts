...
// write the same code but in typescript and using knex.js
...
export class SQLDatabase {
  ...
  constructor({
    ...
    this.inspector = engine.inspect();
    this.allTables = new Set(this.inspector.tables(schema));
  ...
  getSingleTableInfo(tableName: string): string {
    const columns = this.getTableColumns(tableName);
    const columnStr = columns.map((column: any) => {
  ...
  truncateWord(content: any, length: number, suffix = "..."): string {
    ...
    return content.slice(0, length - suffix.length).split(" ").slice(0, -1).join(" ") + suffix;
  }

  runSQL(command: string): Promise<[string, { result: any[]; colKeys: string[] }]> {
    return this.engine.raw(command).then((result: any) => {
      if (result.length > 0) {
        const truncatedResults = result.map((row: any) =>
          row.map((column: any) =>
            this.truncateWord(column, this.maxStringLength)
          )
        );
        return [JSON.stringify(truncatedResults), { result: truncatedResults, colKeys: Object.keys(result[0]) }];
      }
...
//     private readonly maxStringLength: number = 300
//   ) {}

//   private get schemaPrefix() {
//     return this.schema ? `${this.schema}.` : "";
//   }

//   private getTableColumns(tableName: string): Promise<ColumnMetadata[]> {
//     return this.knex(tableName).columnInfo();
//   }

//   private getUsableTableNames(): Promise<string[]> {
//     return this.knex
//       .raw(
//         `SELECT table_name
...
//   private getSingleTableInfo(tableName: string): Promise<string> {
//     return this.getTableColumns(tableName).then((columns) => {
...
//   async insertIntoTable(tableName: string, data: Record<string, any>): Promise<void> {
//     await this.knex(tableName).insert(data);
//   }

//   private truncateWord(content: any, length: number, suffix = "..."): string {
//     if (typeof content !== "string" || length <= 0) {
//       return content;
//     }
...
//   async runSQL(command: string): Promise<[string, { result: any[]; colKeys: string[] }]> {
//     try {
//       const result = await this.knex.raw(command);
//       if (result.length > 0) {
//         const truncatedResults = result.map((row) =>
//           row.map((column) =>
//             this.truncateWord(column, this.maxStringLength)
//           )
//         );
...
//       }
//     } catch (error) {
//       throw new Error(`Statement ${command} is invalid SQL.`);
//     }
//     return ["", { result: [], colKeys: [] }];
//   }

//   async getTableInfo(tableName: string): Promise<string> {
//     return this.getSingleTableInfo(tableName);
//   }

//   async getTableInfoBatch(tableNames: string[]): Promise<Record<string, string>> {
//     const tableInfo: Record<string, string> = {};
//     for (const tableName of tableNames) {
//       tableInfo[tableName] = await this.getSingleTableInfo(tableName);
//     }
//     return tableInfo;
//   }

//   async getTableInfoAll(): Promise<Record<string, string>> {
//     const tableNames = await this.getUsableTableNames();
//     return this.getTableInfoBatch(tableNames);
//   }
// }