...
import {SimplePrompt, buildToolsText} from "./Prompt";
...
export interface BaseSelector {
  ...
  select(query: string, metadata: ToolMetadata[], num_selections?: number): Promise<Selection[]>;
}
...
export class LLMSelector implements BaseSelector {
  ...
  async select(query: string, metadata: ToolMetadata[], num_selections: number): Promise<Selection[]>{

    //Depending on if we have the single or multi-select case, change input object to prompt.
    var input;
    if(num_selections > 1){
      input = {context: buildToolsText(metadata), query: query, branching_factor: num_selections.toString()};
    }
    else{
      input = {context: buildToolsText(metadata), query: query};
    }
    const selections: Selection[]  = parseJsonMarkdown((await this.llm.complete(this.prompt(input))).message.content);
    return selections;
  }
}