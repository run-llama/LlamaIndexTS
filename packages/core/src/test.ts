import { Document, OpenAI, SimpleNodeParser, SummaryExtractor } from "./index";
...
(async () => {
  ...
  if (!nodesWithSummaryMetadata) {
    throw new Error("No nodes with summary metadata");
  }

  for (const node of nodesWithSummaryMetadata) {
    node.metadata.sectionSummary;
  }
