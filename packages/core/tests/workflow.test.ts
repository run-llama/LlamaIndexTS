...
import { Workflow, type LegacyContext } from "../src/workflow/workflow.js";
...
describe("Workflow", () => {
  ...
  beforeEach(() => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    generateJoke = vi.fn(async (_context, ev: StartEvent<any>) => {
    ...
    analyzeJoke = vi.fn(async (_context: HandlerContext, ev: JokeEvent) => {
  ...
  test("workflow timeout", async () => {
    ...
    const longRunning = async (_context: LegacyContext, ev: StartEvent) => {
  ...
  test("collectEvents", async () => {
    let collectedEvents: WorkflowEvent<unknown>[] | null = null;
    const jokeFlow = new Workflow({ verbose: true });

    jokeFlow.addStep(StartEvent, generateJoke);
    jokeFlow.addStep(JokeEvent, analyzeJoke);
    jokeFlow.addStep([AnalysisEvent], async (context, ev) => {
      collectedEvents = context.data.collectEvents(ev, [AnalysisEvent]);
