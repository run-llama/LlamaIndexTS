...
const getVectorDbChoices = (framework) => {
  const choices = [
    {
...
const getDataSourceChoices = (framework) => {
  const choices = [
    {
...
const selectLocalContextData = async (type) => {
  try {
    let selectedPath = "";
    let execScript;
    let execOpts = {};
...
export const askQuestions = async (program, preferences) => {
  var _a;
  const getPrefOrDefault = (field) => {
    var _a;
    return (_a = preferences[field]) !== null && _a !== void 0
      ? _a
      : defaults[field];
  };
  ...
  if (
    ((_a = program.dataSource) === null || _a === void 0 ? void 0 : _a.type) ===
      "web" &&
    program.framework === "fastapi"
  ) {
  ...
  if (
    ...
    if (ciInfo.isCI) {
      ...
      const tools =
        toolsName === null || toolsName === void 0
          ? void 0
          : toolsName.map((tool) =>
