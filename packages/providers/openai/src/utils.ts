...
export function isReasoningEffortSupported(model: ChatModel | string, effort: string | undefined): boolean {
  const supportedReasoningEffort = ["low", "medium", "high", undefined]
  return model.startsWith("gpt-5") ? [...supportedReasoningEffort, "minimal"].includes(effort) : supportedReasoningEffort.includes(effort);
}
