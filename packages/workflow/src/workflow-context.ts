import {
	...
	StartEvent, type StartEventConstructor,
	StopEvent, type StopEventConstructor,
	WorkflowEvent
} from './workflow-event';
...
) => Promise<InstanceType<Out[number]>>;
...
function flattenEvents (
	...
	const eventMap = new Map<AnyWorkflowEventConstructor, WorkflowEvent<unknown>>();
...
export class WorkflowContext<
	...
	Stop = string,
	Data = unknown
> implements AsyncIterable<WorkflowEvent<unknown>, unknown, void>,
	...
	#stepCache: WeakMap<
		WorkflowEvent<unknown>,
		[step: Set<StepHandler<Data, never, never>>, stepInputs: WeakMap<StepHandler<Data, never, never>, AnyWorkflowEventConstructor[]>]
	> = new Map();
	...
	#getStepFunction (
		...
		stepInputs: WeakMap<StepHandler<Data, never, never>, AnyWorkflowEventConstructor[]>,
	] {
		if (this.#stepCache.has(event)) {
			return this.#stepCache.get(event)!;
		}
		const set = new Set<StepHandler<Data, never, never>>();
		const stepInputs = new WeakMap<StepHandler<Data, never, never>, AnyWorkflowEventConstructor[]>();
		const res: [
			step: Set<StepHandler<Data, never, never>>,
			stepInputs: WeakMap<StepHandler<Data, never, never>, AnyWorkflowEventConstructor[]>,
		] = [set, stepInputs];
						...
						switch (eventProtocol.type) {
							...
							case 'event': {
								...
								if (isPendingEvents.has(event)) {
									...
									const nextEventPromises: Promise<WorkflowEvent<unknown>>[] = [...steps].map(
										(step) => {
											...
											return (step as StepHandler<Data>).call(
												null,
												{
													get data () {
														return data;
