...
- `pagePrefix?` Optional. A templated prefix to add to the beginning of each page. If it contain `{page_number}`, it will be replaced by the page number.
- `pageSuffix?` Optional. A templated suffix to add to the end of each page. If it contain `{page_number}`, it will be replaced by the page number.
