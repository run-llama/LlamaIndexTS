...
For the parameters of the sum function, we can define a JSON schema or a Zod schema.
...
const sumArgsSchema = z
  .object({
    a: z.number().describe("The argument a to sum"),
    b: z.number().describe("The argument b to sum"),
  })
