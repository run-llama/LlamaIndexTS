...
See [LlamaParseReader.ts](https://github.com/run-llama/LlamaIndexTS/blob/060b700e0941f0a7668b31d758ff7d2d1685f5bc/packages/core/src/readers/LlamaParseReader.ts#L6) for a list of supported file types:
...
`language` primarly helps with OCR recognition. Defaults to `en`. See [../readers/type.ts](https://github.com/run-llama/LlamaIndexTS/blob/060b700e0941f0a7668b31d758ff7d2d1685f5bc/packages/core/src/readers/type.ts#L20) for a list of supported languages.
...
`invalidateCache` set to true to ignore the LlamaCloud cache. All document are kept in cache for 48hours after the job was completed to avoid processing the same document twice. Can be useful for texting when trying to re-parse the same document with, e.g. different `parsingInstructions`.
