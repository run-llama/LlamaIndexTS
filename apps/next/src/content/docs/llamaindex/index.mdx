...
[Agents](./modules/agents/index.mdx) are LLM-powered assistants that can reason, use external tools, and take actions to accomplish tasks such as research, data extraction, and automation. LlamaIndex.TS provides foundational building blocks for creating and orchestrating these agents.

### What are workflows?

[Workflows](./understanding/workflows/index.mdx) are multi-step, event-driven processes that combine agents, data connectors, and other tools to solve complex problems. With LlamaIndex.TS you can chain together retrieval, generation, and tool-calling steps and then deploy the entire pipeline as a microservice.

### What is context engineering?

LLMs come pre-trained on vast public corpora, but not on **your** private or domain-specific data. Context engineering bridges that gap by injecting the right pieces of your data into the LLM prompt at the right time. The most popular example is [Retrieval-Augmented Generation (RAG)](./getting_started/concepts.mdx), but the same idea powers evaluation, extraction, summarisation, and more.

LlamaIndex.TS gives you:

- **Data connectors** to ingest from APIs, files, SQL, and dozens more sources.
- **Indexes & retrievers** (vector, keyword, summary, hybrid…) optimised for LLM consumption.
- **Engines** — query, chat, and tool interfaces over your data.
- **Agents & workflows** for higher-level reasoning and orchestration.
...
- [Question-Answering (RAG)](./use_cases/q_and_a/index.mdx)
- [Chatbots](./use_cases/chatbots.mdx)
- [Document understanding & extraction](./use_cases/extraction.mdx)
- [Autonomous research agents](./use_cases/agents.mdx)
- [Multi-modal apps](./use_cases/multimodal.mdx)
- [Fine-tuning & eval](./use_cases/fine_tuning.mdx)

Explore more in [our use-case gallery](./use_cases/index.mdx).
...
Want to play first? Spin up the starter in StackBlitz — no local setup required:
...
Need an end-to-end managed pipeline? Check out **[LlamaCloud](https://llamaindex.ai/enterprise)**: best-in-class document parsing (LlamaParse), extraction (LlamaExtract), and indexing services with generous free tiers.
...
- [LlamaHub](https://llamahub.ai) — a growing collection of connectors & integrations.
