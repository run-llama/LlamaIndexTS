...
// Get the ephemeral key on the server 
const serverllm = openai({
  apiKey: "your-api-key", 
  model: "gpt-4o-realtime-preview-2025-06-03",
});

// Get an ephemeral key for secure client-side usage
...
// Create a live session with audio support
const session = await llm.live.connect({
  systemInstruction: "You are a helpful assistant.",
  // Enable both text and audio responses
  responseModality: [ModalityType.TEXT, ModalityType.AUDIO],
  // Audio configuration is handled automatically by LlamaIndex
  audioConfig: {
    stream: userStream,
    onTrack: (remoteStream) => {
      // LlamaIndex will handle WebRTC setup, ICE candidates,
      // and audio track management automatically
...
// Send a message - audio will be streamed automatically
session.sendMessage({
  content: "Hello!",
  role: "user",
});
...
> **Note**: LlamaIndex handles all the complex WebRTC setup, audio track management, and real-time streaming internally. You only need to:
> 1. Provide the user's audio stream
> 2. Handle the incoming remote stream in the `onTrack` callback
> 3. Everything else (ICE candidates, peer connections, data channels) is managed for you
