...
- `vectorBlock`: A memory block that stores and retrieves batches of chat messages from a vector database.
...
const memoryBlocks= [
  ...
  vectorBlock({
    ...
    embedModel: new OpenAIEmbedding({ model: "text-embedding-ada-002" }),
    priority: 2,
  }),
];
```
...
- `vectorBlock`: A vector memory block that will store and retrieve batches of chat messages from a vector database. Each batch is a list of the flushed chat messages. Here we've passed in the `vectorStore` and `embedModel` to use to store and retrieve the chat messages.
