...
- `vectorBlock`: A vector memory block that will store and retrieve chat messages from a vector database using semantic similarity search. Messages are stored individually and retrieved based on their relevance to recent conversation context. Here we've passed in the `vectorStore` and `embedModel` to use to store and retrieve the chat messages.
