...
```javascript
const workflowContext = workflow.run("Sum 101 and 303");
...
```javascript
const workflowContext = workflow.run("Add 101 and 303");
for await (const event of workflowContext) {
...
```javascript
const workflowContext = workflow.run("Sum 202 and 404");
for await (const event of workflowContext) {
  if (event instanceof AgentStream) {
    // Stream the response
    for (const chunk of event.data.delta) {
