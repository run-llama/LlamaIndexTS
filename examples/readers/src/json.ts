...
import { JSONReader } from "../../../packages/llamaindex/src/readers/JSONReader.js";
...
async function main() {
  ...
  const reader6 = new JSONReader({ ensureAscii: true, verbose: true });
  const docs6 = await reader6.loadDataAsContent(nonAsciiBuffer);
  console.log(docs6[0]);

  // JSON Lines Format
  const reader7 = new JSONReader({
    isJsonLines: true,
